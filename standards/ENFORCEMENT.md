...
A bounded watch re-entry is `python3 tools/coordinator_watch.py evaluate --facts <facts.json> --watch-state <state.json>`. The evaluator calls the planner and emits one result. It does not spawn processes, call GitHub, merge, send Telegram, or start/stop Cursor. Ambiguous mutation facts yield `BLOCK_RECONCILIATION` and authorize no retry. Host scheduling and delivery stay outside this evaluator.
