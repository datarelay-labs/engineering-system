...
`python3 tools/benchmark_execution.py dry-run` prepares the frozen BENCH-BUG-001 pilot only. It binds the control head, candidate head, manifest head, fixture id, and task-source head, then gives each lane the sanitized `worker_task()` projection plus run metadata. Duration, usage, and the shared retry/rework counts reuse efficiency-telemetry semantics: unexposed cost and usage stay unknown, and the tool does not write a second telemetry record. A provider, model, reasoning, or toolset mismatch is `PARTIAL` or `BLOCK`. The result skeleton terminal stays `BLOCK` with `DRY_RUN_NOT_EXECUTED`, so preparation cannot be reported as a comparable benchmark pass. The dry-run does not launch a model or agent worker.
