...
`python3 tools/benchmark_execution.py dry-run` prepares one of the seven frozen cases. `--case-id` selects the case and defaults to BENCH-BUG-001. A case id outside that frozen set is rejected. It binds the shared control head and candidate head, the frozen manifest head, and the selected case's repository, fixture id, and task-source head, then gives each lane the same sanitized `worker_task()` projection plus lane run metadata. The lane artifact is a non-final result template: evaluator-owned #44 fields stay null, and unavailable wall time and model cost stay `UNKNOWN`. Observed duration, usage, counts, and exact-head evidence are derived only from a schema-valid canonical efficiency telemetry record whose repository, case/lane workstream, run id, and profile match the selected case and lane, and whose `validation.exact_head` matches that lane's system head whenever exact-head evidence is present. Telemetry from another case, lane, repository, run, profile, or head is rejected. A dry-run may stamp the frozen manifest revision only when the loaded manifest content is the blob at that revision. A final result accepts one frozen case only when its case id, fixture id, and either the control version/head pair or the candidate version/head pair agree. A result for one case or lane does not bind to another. #44 `REVIEW_REWORK` uses `efficiency_telemetry.rework_count`, the same P0b total of `pr_rework`, `ci_rework`, and `review_rework` that an outcome report records as `rework_count`. The dry-run does not emit a second telemetry record. A provider, model, reasoning, or toolset mismatch is `PARTIAL` or `BLOCK`. The template is not a final benchmark result. Comparable lanes bind requested launch sandbox `enabled` separately from observed effective sandbox `disabled`; the toolset is not that sandbox. Persistent lanes finalize counts only through `tools/efficiency_telemetry.py` after a qualified native hook receipt. A native `stop` hook checkpoints one agent loop and does not seal the lane. Explicit finalization requires that latest loop to be completed, derives counters once, writes the canonical record, and then deletes ephemeral correlation state. Count-relevant hooks after that seal fail closed. The dry-run does not launch a model or agent worker.
