...
- Persistent benchmark lanes use the hook-only plugin `benchmarks/persistent-instrumentation` outside the historical task tree. Native hook JSON is the only event source. Requested launch sandbox and observed effective sandbox stay separate. Preparation admits only a clean checkout of the frozen repository identity and exact task HEAD; dirty, staged, untracked, or submodule drift blocks before lane state exists. The effective toolset is the deny-by-default hook surface plus the absence of denied executions, not the descriptor string and not a requirement to exercise every allowed tool. A native `stop` hook checkpoints one agent loop and keeps keyed correlation state while the lane is open. Explicit finalization writes the canonical record only when a host-signed assertion verifies against the fixed root-administered trust anchor and binds the run, lane, descriptor, receipt digest, task identity, system head, and finalization state. A caller witness, worker-writable receipt, hidden path, file mode, or environment secret is not that authority; missing anchor or signature fails closed and does not record telemetry. This module does not mint the signing key. Missing usage and cost stay null. Do not estimate.
