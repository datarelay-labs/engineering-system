...
- Persistent benchmark lanes use the hook-only plugin `benchmarks/persistent-instrumentation` outside the historical task tree. Native hook JSON is the only event source. Requested launch sandbox and observed effective sandbox stay separate. Preparation admits only the frozen repository identity and exact task HEAD. The effective toolset is the observed hook surface, not the descriptor string. A native `stop` hook checkpoints one agent loop and keeps keyed correlation state while the lane is open. Explicit finalization writes the canonical record only when a root-owned host receipt anchor is present. A worker-writable receipt, hidden path, file mode, or environment secret is not that authority; a missing anchor fails closed and does not record telemetry. Missing usage and cost stay null. Do not estimate.
