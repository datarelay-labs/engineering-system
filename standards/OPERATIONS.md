...
Additive capabilities are per-worktree `start`, focused `logs`, `browser` screenshot evidence, `metrics`, `traces`, and `cleanup`. Each entry is explicitly `supported` or `unsupported`. Unsupported entries contain no command. Supported entries require `scope: worktree` and a local command. Logs, browser, metrics, and traces also require `fuller_command` so complete evidence has a deterministic path. Commands must not use `sudo`, remote URLs, credential assignments, shell substitution, redirection, or parent-directory traversal. `python3 tools/runtime-contract.py check` validates and prints the resolved commands; it does not execute them and it does not require an observability vendor.
